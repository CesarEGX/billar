...
<html lang="en">
    ...
    <?php
        ...
        if($resultado -> num_rows > 0){
            while($dataUsuario = $resultado -> fetch_assoc()){
                $nombre_producto = $dataUsuario['nombre'];
                $stock = $dataUsuario['stock'];
                $id_tipo_productoLF = $dataUsuario['id_tipo_productoLF'];//categoria
                $id_marcaLF = $dataUsuario['id_marcaLF'];
            }
        }
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 style="margin-bottom:2rem;" class="mt-4">Editar</h1>
                            ...
                            <div class="card-body">
                                <div class="card mb-4">
                                    <div class="card-body">
                                        ...
                                        <span>-No se puedra eliminar productos que tengan registros con las reservas .</span></br>
                                    </div>
                                </div>
                                <div class="">
                                ...
                                <div class="formularioAdd">
                                    <form id="formularioUsuario" class="needs-validation" novalidate onsubmit="editarProducto(event)" >
                                        <div class="form-row">
                                            <input type="hidden" id="id_producto" value="<?php echo $id_producto; ?>" >
                                            <div class="form-group col-md-6">
                                                <label for="inputEmail4">Nombre del producto</label>
                                                <input type="text" class="form-control inputUser" id="inputProducto" required value="<?php echo $nombre_producto; ?>">
                                                <div class="invalid-feedback">Por favor ingresa un producto válido</div>
                                            </div>

                                            <div class="form-group col-md-6">
                                                <label for="inputCi">Marca</label>
                                                <?php
                                                    $sql = "SELECT * FROM marca";
                                                    $resultado = mysqli_query($conexion,$sql);
                                                ?>

                                                <select class="form-control inputUser" id="inputMarca" required>
                                                    <?php
                                                        if($resultado -> num_rows > 0){
                                                            while($row = $resultado -> fetch_assoc()){
                                                                $selected = $row["id_marca"] == $id_marcaLF ? 'selected' : '';
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-6">
                                                    <label for="inputCi">Categoria</label>
                                                    <?php
                                                        $sql = "SELECT * FROM tipo_producto";
                                                        $resultado = mysqli_query($conexion,$sql);
                                                    ?>

                                                <select class="form-control inputUser" id="inputTipoProducto" required>
                                                    <?php
                                                        if($resultado -> num_rows > 0){
                                                            while($row = $resultado -> fetch_assoc()){
                                                                $selected = $row["id_tipo_producto"] == $id_tipo_productoLF ? 'selected' : '';
                                                                echo "<option value='".$row["id_tipo_producto"]."' ".$selected.">".$row["descripcion"]."</option>";
                                                            }
                                                        }
                                                    ?>
                                                </select>
                                                    <div class="invalid-feedback">Por favor ingresa una categoria válida.</div>
                                            </div>
                                            <div class="form-group col-md-6">
                                                <label for="inputEmail4">Cantidad</label>
                                                <input type="number" class="form-control inputUser" id="inputCantidad" required value="<?php echo $stock; ?>">
                                        ...
                                        <div style="display:flex;flex-direction:row;margin-top:2.5rem;" class="">
                                            ...
                                            <a href="../producto.php" style="margin-left:2rem;" class="btn btn-dark">Cancelar</a>
                                        </div>
                                    </form>
                                </div>
                            </div>
